...
public class HarborSceneUIManager : MonoBehaviour
{
    ...
    [Header("Data (drag ScriptableObjects)")]
    ...
    [SerializeField] private TMP_Text pointsLabel; // binds to LABEL_CurrentPoints/TEXT_PointsLabel if present
    ...
    private int GetTier(UpgradeCategory cat, ShipType ship) => PlayerPrefs.GetInt(TierKey(cat, ship), 0);
    ...
    private void Awake() { TryFindUIRefs(transform); }
    private void Start()
    {
    ...
    public void ToBattle()
    {
        ...
        catch { /* fall through to Unity fallback */ }

        // Fallbacks using Unity SceneManager
        if (Application.CanStreamedLevelBeLoaded("Game")) { USM.LoadScene("Game"); return; }
    ...
    {
        var path = t.name;
        while (t.parent && t.parent != stopAt) { t = t.parent; path = t.name + "/" + path; }
        return path;
    }
    private static object GetIndex(IEnumerable enumerable, int index)
    {
        if (index < 0) return null;
        int i = 0; foreach (var o in enumerable) { if (i++ == index) return o; }
        return null;
    }
